  ...
  return (
    <div>
      <form>
        ...
        <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
          {Object.keys(images).map((key) => (
            <label htmlFor={`roomImage${key}`} key={key}>
              <img className='max-h-14 cursor-pointer opacity-80'
        ...
        <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
          <div className='flex-1 max-w-50'>
            ...
            <select value={inputs.roomType} onChange={e => setInputs({ ...inputs, roomType: e.target.value })}
              className='border opacity-80 border-gray-400 mt-2 rounded p-2 w-full'>
          ...
          <div>
            ...
            <input type="number" placeholder='0' className='border border-gray-500 mt-2 rounded p-2 w-26' />
          </div>
        </div>

        <p className='text-gray-700 mt-4'>Amenities</p>
        <div className='flex flex-col flex-wrap mt-1 text-gray-300 max-w-sm'>
        ...
        <button className='bg-gray-600 hover:bg-black text-white px-8 py-2 transition-all ease-in-out duration-400 rounded mt-8'>
          Add Room
        </button>
      </form>
    </div>
